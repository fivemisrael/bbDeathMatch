...
function openGui()
  ...
  TriggerEvent("banking:updateBank")
end
    ...
    if IsNearATM() then
      if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
          ...
          if ( atmuse ) then 
              ...
              atmuse = false
              local finished = exports["bb-taskbar"]:taskBar(3000,"Retrieving Card")
              ClearPedTasks(PlayerPedId())
          else
              atmuse = true
        ...
        if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
            ...
            if ( atmuse ) then 
                ...
                atmuse = false
                local finished = exports["bb-taskbar"]:taskBar(1000,"Retrieving Card")
                ClearPedTasks(PlayerPedId())
            else
                atmuse = true
                TaskPlayAnim( player, "mp_common", "givetake1_a", 1.0, 1.0, -1, 49, 0, 0, 0, 0 )
...
RegisterNUICallback('quickCash', function(data, cb)
  TriggerEvent('bank:withdbbobsfciorraw', 100)
  TriggerEvent("banking:updateBank")
  cb('ok')
end)

RegisterNUICallback('withdrawSubmit', function(data, cb)
  TriggerEvent('bank:withdbbobsfciorraw', data.amount)
  TriggerServerEvent('banking:updateBank')
  cb('ok')
end)

RegisterNUICallback('depositSubmit', function(data, cb)
  TriggerEvent('bank:depbbobsfciorosit', data.amount)
  TriggerEvent("banking:updateBank")
  cb('ok')
end)
...
RegisterNUICallback('transferSubmit', function(data, cb)
  local fromPlayer = GetPlayerServerId();
    TriggerServerEvent('bank:tranbbobsfciorsfer', data.to, data.amount)
    TriggerEvent("banking:updateBank")
  cb('ok')
end)
...
RegisterNetEvent('bank:depbbobsfciorosit')
AddEventHandler('bank:depbbobsfciorosit', function(amount)
  if(IsNearBank() == true ) then
    ...
    TriggerEvent('notification', 'You cannot use the ATM in a vehicle.', 2, 3500)
      ...
      TriggerServerEvent("bank:depbbobsfciorosit", tonumber(amount))
    end
  else
...
RegisterNetEvent('bank:withdbbobsfciorraw')
AddEventHandler('bank:withdbbobsfciorraw', function(amount)
  if(IsNearATM() == true or IsNearBank() == true) then
    if (IsInVehicle()) then
    else
      TriggerServerEvent("bank:withdbbobsfciorraw", tonumber(amount))
    end
  end
end)
...
RegisterNUICallback('transfer', function(data)
  TriggerServerEvent('bank:tranbbobsfciorsfer', data.to, data.amountt)
end)
...
RegisterNetEvent('bank:tranbbobsfciorsfer')
AddEventHandler('bank:tranbbobsfciorsfer', function(to, amount)
  local player2 = GetPlayerFromServerId(toPlayer)
  local playing = IsPlayerPlaying(player2)
  if (playing ~= false) then
    TriggerServerEvent("bank:tranbbobsfciorsfer", toPlayer, tonumber(amount))
  else
    TriggerEvent('chatMessage', "", {255, 0, 0}, "^1This player is not online!");
  end
end)
...
AddEventHandler('banking:updateBank', function(show)
  local id = PlayerId()
  local name = GetPlayerName(id)
  ESX.TriggerServerCallback('bankk:getbankamount', function(moneyAmount)
    SendNUIMessage({
      updateBalance = true,
      balance = moneyAmount,
      name = name,
