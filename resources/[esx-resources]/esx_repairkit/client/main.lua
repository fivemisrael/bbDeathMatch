	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
		...
		if DoesEntityExist(vehicle) then
			...
			Citizen.CreateThread(function()
				...
				local finished = exports["bb-taskbar"]:taskBar(10000,"Repairing")
				if CurrentAction ~= nil then
					...
					TriggerEvent('notification', 'You repaired the vehicle.', 1)
				end
		...
		Citizen.CreateThread(function()
			...
			if CurrentAction ~= nil then
				...
				if IsControlJustReleased(0, Keys["X"]) then
					...
					TriggerEvent('notification', 'You aborted the repair.', 1)
					CurrentAction = nil
				end
			end
		end)
	else
		TriggerEvent('notification', 'There is no vehicle nearby.', 2)
	end
end)
