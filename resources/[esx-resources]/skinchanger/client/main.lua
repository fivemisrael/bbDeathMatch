...
local hasDonor = false

RegisterNetEvent("skinchanger:checkDonor")
AddEventHandler("skinchanger:checkDonor", function(donor)
	hasDonor = donor
end)

CreateThread(function()
	TriggerServerEvent("skinchanger:checkDonor")
end)
...
--crun hash = GetHashKey("ig_josh") RequestModel(hash) Wait(500) SetPlayerModel(PlayerId(), hash) SetModelAsNoLongerNeeded(hash)
