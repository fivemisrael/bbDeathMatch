...
RegisterNetEvent('esx_vehicleshop:setVehicleOwnedbbobsfciorPlayerId')
AddEventHandler('esx_vehicleshop:setVehicleOwnedbbobsfciorPlayerId', function(playerId, vehicleProps, model, label)
				...
				MySQL.Async.execute('DELETE FROM cardealer_vehicles WHERE id = @id', {
					...
					if rowsChanged == 1 then
						MySQL.Async.execute('INSERT INTO owned_vehicles (owner, plate, vehicle) VALUES (@owner, @plate, @vehicle)', {
							...
							--TriggerClientEvent('notification', xPlayer.source, )
...
ESX.RegisterServerCallback('esx_vehicleshop:getVehiclePrice', function(source, cb, modelname)
    local modelPrice
	for k,v in ipairs(vehicles) do
		print(v.model)
		if string.lower(modelname) == v.model then
			modelPrice = v.price
			break
		end
	end
