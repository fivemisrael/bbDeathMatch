...
AddEventHandler("esx_inventoryhud:openMotelsInventoryBed", function(data, owner, motel, room)
...
RegisterNUICallback(
    ...
    function(data, cb)
        ...
        if type(data.number) == "number" and math.floor(data.number) == data.number then
            ...
            TriggerServerEvent("pw-motels:putBedItem", ESX.GetPlayerData().identifier, data.item.type, data.item.name, count, hotelMotel, hotelRoom)
        end
...
RegisterNUICallback(
    ...
    function(data, cb)
        ...
        if type(data.number) == "number" and math.floor(data.number) == data.number then
            TriggerServerEvent("pw-motels:getBedItem", ESX.GetPlayerData().identifier, data.item.type, data.item.name, tonumber(data.number), hotelMotel, hotelRoom)
        end
